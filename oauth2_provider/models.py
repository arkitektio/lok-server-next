...
class ClientSecretField(models.CharField):
    def pre_save(self, model_instance, add):
        ...
        try:
            ...
            logger.debug(
                f"{model_instance}: {self.attname} is already hashed with {hasher}."
            )
        except ValueError:
            logger.debug(
                f"{model_instance}: {self.attname} is not hashed; hashing it now."
            )
...
class AbstractApplication(models.Model):
    ...
    * :attr:`name` Friendly name for the Application
    ...
    algorithm = models.CharField(
        max_length=5, choices=ALGORITHM_TYPES, default=NO_ALGORITHM, blank=True
    )
    ...
    def clean(self):
            ...
            for uri in redirect_uris:
                ...
                if scheme not in allowed_schemes:
                    raise ValidationError(
                        _("Unauthorized redirect scheme: {scheme}").format(
                            scheme=scheme
                        )
                    )
        ...
        if self.algorithm == AbstractApplication.RS256_ALGORITHM:
            if not oauth2_settings.OIDC_RSA_PRIVATE_KEY:
                raise ValidationError(
                    _("You must set OIDC_RSA_PRIVATE_KEY to use RSA algorithm")
                )
        ...
        if self.algorithm == AbstractApplication.HS256_ALGORITHM:
            if any(
                ...
                raise ValidationError(
                    _("You cannot use HS256 with public grants or clients")
                )
        ...
        if self.algorithm == AbstractApplication.RS256_ALGORITHM:
            if not oauth2_settings.OIDC_RSA_PRIVATE_KEY:
                raise ImproperlyConfigured(
                    "You must set OIDC_RSA_PRIVATE_KEY to use RSA algorithm"
                )
...
def clear_expired():
    def batch_delete(queryset, query):
        CLEAR_EXPIRED_TOKENS_BATCH_SIZE = (
            oauth2_settings.CLEAR_EXPIRED_TOKENS_BATCH_SIZE
        )
        CLEAR_EXPIRED_TOKENS_BATCH_INTERVAL = (
            oauth2_settings.CLEAR_EXPIRED_TOKENS_BATCH_INTERVAL
        )
        current_no = start_no = queryset.count()

        while current_no:
            flat_queryset = queryset.values_list("id", flat=True)[
                :CLEAR_EXPIRED_TOKENS_BATCH_SIZE
            ]
            batch_length = flat_queryset.count()
            queryset.model.objects.filter(id__in=list(flat_queryset)).delete()
            logger.debug(
                f"{batch_length} tokens deleted, {current_no-batch_length} left"
            )
    ...
    if REFRESH_TOKEN_EXPIRE_SECONDS:
        if not isinstance(REFRESH_TOKEN_EXPIRE_SECONDS, timedelta):
            try:
                REFRESH_TOKEN_EXPIRE_SECONDS = timedelta(
    ...
    if refresh_expire_at:
        ...
        logger.info(
            "refresh_expire_at is %s. No refresh tokens deleted.", refresh_expire_at
        )
