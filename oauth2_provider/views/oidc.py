    ...
    def get(self, request, *args, **kwargs):
        ...
        if not issuer_url:
            ...
            userinfo_endpoint = (
                ...
                or request.build_absolute_uri(reverse("oauth2_provider:user-info"))
            )
            ...
            if oauth2_settings.OIDC_RP_INITIATED_LOGOUT_ENABLED:
                end_session_endpoint = "{}{}".format(
                    host, reverse("oauth2_provider:rp-initiated-logout")
                )
...
def _validate_claims(request, claims):
    ...
    if "iss" not in claims or claims["iss"] != validator.get_oidc_issuer_endpoint(
        request
    ):
...
def validate_logout_request(
    ...
    prompt_logout = (
        must_prompt_logout or oauth2_settings.OIDC_RP_INITIATED_LOGOUT_ALWAYS_PROMPT
    )
...
class RPInitiatedLogoutView(OIDCLogoutOnlyMixin, FormView):
    ...
    def get_initial(self):
        return {
            ...
            "post_logout_redirect_uri": self.oidc_data.get(
