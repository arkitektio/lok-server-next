...
if TYPE_CHECKING:
    ...
    from contrib.backends.docker_backend import (
...
def _create_base_url(
    ...
    inside_base_url = (
        f"{protocol}://{descriptor.internal_host}:{inside_port.split('/')[0]}"
    )
    outside_base_url = (
        f"{protocol}://{context.request.host}" + f"/{descriptor.internal_host}"
    )
...
def lok(
    ...
    return {
        ...
        "grant_type": context.client.authorization_grant_type,
...
def rekuest(
    ...
    return generic(self, context, descriptor) | {
        "agent": {"endpoint_url": ws_base_url + "/agi"}
    }
