...
class ConfigBackend(BackendBase):
    ...
    def __init__(self, config: dict) -> None:
        ...
        ), f"Instance directory {self.instance_dir} does not exist"
        ...
        for instance_file in self.instance_dir.iterdir():
            if instance_file.is_file():
                try:
                    ...
                    result = yaml.load(
                        Template(template).render(fake_context), Loader=yaml.SafeLoader
                    )
                    ...
                    assert (
                        service_config
                    ), f'Service file {instance_file} does not contain an service identifier ("__service")'
                    ...
                    logger.info(
                        f"Loaded instance {instance_file.stem} for service {service_descriptor.identifier}"
                    )

                except Exception as e:
                    raise ValueError(
                        f"Error loading instance {instance_file}: {e}"
                    ) from e
    ...
    def render(self, instance_id: str, linking: LinkingContext) -> Dict[str, Any]:
        ...
        answer = yaml.load(
            Template(template).render(linking.dict()), Loader=yaml.SafeLoader
        )
        return answer
