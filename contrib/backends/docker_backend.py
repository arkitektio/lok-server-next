...
class DockerBackend(BackendBase):
    ...
    def __init__(self, config_values):
        ...
        try:
            ...
            raise Exception(
                "Could not connect to Docker. Did you mount the socket?"
            ) from e
    ...
    def rescan(self):
        ...
        sibling_containers = retrieve_sibling_containers(
            self.client, self.self_project_name
        )
            ...
            if self._is_fakts_enabled(container):
                ...
                if service_identifier in loading_services:
                    ...
                    if service_descriptor.description is None:
                        service_descriptor.description = labels.get(
                            "fakts.service.description"
                        )
                    if service_descriptor.validator is None:
                        service_descriptor.validator = labels.get(
                            "fakts.service.validator"
                        )
                else:
                ...
                assert (
                    builder in self.loaded_builders
                ), f"Builder {builder} not found. Cannot register instance"

                # Overwriting the instance if it already exists is okay, because
                # the configuration is done on the service level, however some
                loading_instances[instance_descriptor.instance_identifier] = (
                    instance_descriptor
                )
                ...
                if (
                    instance_descriptor.instance_identifier
                    in loading_service_descriptors
                ):
                    # Service already loaded just updating the labels if they are
                    # not already set
                    instance_descriptor = loading_service_descriptors[
                        instance_descriptor.instance_identifier
                    ]
    ...
    def render(self, service_instance: str, context: LinkingContext) -> Dict[str, Any]:
        ...
        assert (
            service_instance in self.loaded_docker_service_descriptors
        ), f"Service instance {service_instance} (after retry) not found"
        service = self.loaded_docker_service_descriptors[service_instance]

        assert (
            service.builder in self.loaded_builders
        ), f"Builder {service.builder} not found in loaded builders"
        ...
        try:
            ...
            raise Exception(
                f"Could not render service {service_instance} with {service.builder}"
            ) from e
